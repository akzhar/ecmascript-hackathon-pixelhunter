...
describe(`Сalc right asnwers`, () => {
  it(`should return 1`, () => {
    assert.equal(1, getRightAnswersCount(notAllAnswers));
  });
  it(`should return 10`, () => {
    assert.equal(10, getRightAnswersCount(allAnswersNotFastNotSlow));
  });
});

describe(`Сalc speed bonuses`, () => {
  it(`should return 1`, () => {
    assert.equal(1, getSpeedBonusCount(notAllAnswers));
  });
});

describe(`Сalc slow penalties`, () => {
  it(`should return 1`, () => {
    assert.equal(1, getSlowPenaltyCount(notAllAnswers));
  });
});
